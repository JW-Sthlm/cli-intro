...
- **GitHub MCP server token is account-locked at install time.** Even after `gh auth switch --user jwallquist_microsoft` and `gh repo view gim-home/pmx-mcp` confirming the EMU account can see the private repo, Copilot CLI's first move was to call the github-mcp-server MCP tool — which still had the personal-account token from when it was installed in 5a. Got 404, searched for alternatives, found Galvill/pmx-mcp, started installing it AGAIN. → Hardened the 5b prompt: explicitly tells Copilot to use shell `git clone` (which respects active gh credentials via `gh auth setup-git`), forbid the github-mcp-server MCP tool, forbid searching for alternative pmx-mcp repos, and forbid installing Galvill/pmx-mcp by name. Added a second callout in 5b warning that the GitHub MCP's token is stale after account switch.
