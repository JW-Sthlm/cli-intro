...
- **EMU sign-in fully blocked on Dev Box (no PIN/Authenticator option, only USB security key).** Some corp tenants enforce phishing-resistant MFA and only register a phone passkey. Dev Boxes have no Bluetooth → phone passkey can't be reached → "Use security key" is the only option, and the user has no USB key. → Added a callout in 2b: skip the EMU login on the Dev Box, run with personal account only, expect Step 3 verify to warn about missing Microsoft account, expect Step 5 PMX install to fail (demo together). Long-term fix documented: register a cross-device passkey via https://mysignins.microsoft.com from a working machine.
